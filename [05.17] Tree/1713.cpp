...
bool cmp(const pair<int, ci> &c1, const pair<int, ci> &c2) {
    if (c1.second.first != c2.second.first) { //두 매개변수의 첫 번째 인자 비교
        return c1.second.first < c2.second.first; //오름차순
    }
    return c1.second.second < c2.second.second; // 두 번째 인자 비교. 오름차순
}
