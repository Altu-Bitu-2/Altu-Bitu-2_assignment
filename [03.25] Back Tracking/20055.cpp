...
void second(int n, int start, int end) {
    ...
    while (cur != start) { //end - 1 부터 start 까지 검사
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            belt[cur].is_on = false; // 로봇을 옮기므로 현재 위치의 로봇 존재 여부는 false
            belt[next].power--; // 옮기려는 위치의 내구도 감소
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) { // 내구도가 0이 되면, 내구도가 0인 칸의 개수 1 증가
                zero_power++;
            }
        }
    }
...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        belt[start].is_on = true; // 현재 위치의 로봇 존재 여부: true
        belt[start].power--; // 로봇이 올라갔으므로 내구도 감소
        if (belt[start].power == 0) { // 내구도가 0이 되면, 내구도가 0인 칸의 개수 증가
            zero_power++;
        }
    }
}
...
int main() {
    ...
    int step = 0;     //단계 수
    while (true) {
        start = minusPosition(n, start); // 벨트 한 칸 회전 시 올리는 위치 인덱스 1 감소
        end = minusPosition(n, end); // 벨트 한 칸 회전 시 내리는 위치 인덱스 1 감소
        ...
        third(start); // 3번 과정
        step++; // 단계 수 증가
        if (zero_power >= k) { // 내구도가 0인 칸의 개수가 k개 이상이면 종료
            break;
        }
    }
