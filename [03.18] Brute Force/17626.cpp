...
int findMinNumber(int n, int num_size, vector<int> &square_num) {
    // square_num: n 이하의 제곱수들을 저장
    // => vector의 마지막 값이 n 이하의 값 중 가장 큰 제곱수
    // => n이 제곱수이면, vector의 마지막 값이 n일것
    if (square_num[num_size - 1] == n) { //1개로 만들 수 있을 경우 바로 리턴
        return 1; // 최소 개수를 구하는 것이므로 바로 return
    }

    // 가장 큰 값들부터 더해서 비교
    for (int i = num_size - 1; i >= 0; i--) { //2개로 만들 수 있는 경우 검사
        for (int j = num_size - 1; j >= 0; j--) {
            // 두 개의 제곱수의 합으로 표현이 되면
    ...
    // 여기까지도 답이 안 나왔으면 자동으로 남은 경우는 4밖에 없으므로 4를 return
    return 4;
}
